...
def test_build_fact_equivalence_pairs_can_include_all_same_segment_pairs(tmp_path: Path) -> None:
    segments = tmp_path / "segments.jsonl"
    references = tmp_path / "references.jsonl"
    candidates = tmp_path / "candidates.jsonl"
    output = tmp_path / "pairs.jsonl"
        ...
        [
            {
                **key,
                "reference_facts": [
                    {"reference_fact_id": "r1", "text": "Alice moved.", "source_turn_ids": [2]}
                ],
            }
        ],
    )
    ...
    rows = [json.loads(line) for line in output.read_text(encoding="utf-8").splitlines()]
    assert len(rows) == 1
    assert rows[0]["shared_source_turn_ids"] == []
    assert manifest["eligible_pair_count"] == 1
    assert manifest["included_no_source_overlap_count"] == 1
    assert manifest["pair_policy"]["source_turn_overlap_required"] is False
